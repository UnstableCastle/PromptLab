...
import { deletePost } from "../../redux/posts/postThunk";
import DeleteConfirmationDialog from "./DeleteConfirmationDiloag";
...
function Profile() {
  ...
  const [confDel, setConfDel] = useState({ show: false, id: null })
    ...
    <>
      ...
      >
        ...
        >
          ...
          <DeleteConfirmationDialog open={confDel.show} onClose={() => setConfDel({ ...confDel, show: false })} onConfirm={() => {
            dispatch(deletePost(confDel.id)).then(v => {
              if (v.meta.requestStatus === "fulfilled") {
                setConfDel({ show: false, id: null })
                dispatch(getPostsByUser({ id: user.id, page: 0, size: 10 }));
              ...
              {myposts.map((cont, index) => (
                <Grid item key={cont.id ?? index} xs={12} sm={6}>
                  <PromptItem
                    showDelete
                    onDelete={() => {
                      setConfDel({ show: true, id: cont.id })
                    }}
                    ...
                    description={
                      cont.promptText || cont.content || cont.description
                    }
