...
import { downloadPostAttachment, toggleUpvote } from "../../redux/posts/postThunk";
...
const PostDetailsDialog = ({
  ...
  const handleDownload = () => {
    dispatch(downloadPostAttachment(post.attachmentUrl)).then((res) => {
  ...
  const handleUpvoteClick = () => {
    if (post?.id) dispatch(toggleUpvote({ route: "postdetails", id: post.id }));
    if (onLike) onLike();
  };
  ...
  const handleProfileClick = () => {
    ...
    if (userId === authorId) {
      ...
      navigate("/user-profile", { state: { id: authorId } });
    }
    onClose();
  };
  ...
  const attachmentUrl = post?.attachmentUrl
    ? `${BACKEND_URL}${post.attachmentUrl}`
    : "";

  const fileName = post?.attachmentUrl?.split("/").pop() || "Attachment";
      ...
      >
        ...
        <DialogContent sx={{ p: 3 }}>
            ...
            >
              ...
              {post?.category && (
                <Chip
                  label={post.category}
                  variant="outlined"
                  sx={{ borderRadius: 2, borderColor: alpha(PRIMARY, 0.25) }}
                />
              )}
            </Stack>
          )}
          ...
          >
            ...
            {/* {
              post.hasUpvoted && <Button
                variant={post.hasUpvoted ? "contained" : "outlined"}
                startIcon={
                  post.hasUpvoted ? <ThumbUpAltIcon /> : <ThumbUpAltOutlinedIcon />
                }
                ...
                sx={{
                  ...
                  textTransform: "none",
                  ...(post.hasUpvoted
                    ? { bgcolor: PRIMARY, "&:hover": { bgcolor: "#4338ca" } }
                    : {
                      color: PRIMARY,
